 ...
 */
 ...
 fn parse_sub_args(args: &[String]) -> Option<Vec<String>> {
     if args.len() > 1 {
         let result: Vec<String> = args[1..].to_vec(); 
         return Some(result);
     }
         ...
         _ => {
             if args[1] == "help" {
                 ...
                 if let Some(sumcommand) = find_sub_command("topicList") {
                     ...
                     sumcommand.excute();
                     let matches = sumcommand.get_arg_matchs();
                     match matches.subcommand() {
                         Some(("topicList", topicList)) => println!(
                             "toplicList contains arg  {}",
                             topicList.get_flag("cluster"),
                         ),
